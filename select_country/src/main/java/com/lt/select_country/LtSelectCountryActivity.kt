 ...
 */
class LtSelectCountryActivity : Activity() {
    private val isEnglish: Boolean by lazy { resources.configuration.locale.language != Locale.CHINA.language }
    ...
    private fun initData() {
        ...
        try {
            ...
            runOnUiThread {
                rvRight.layoutManager =
                    LinearLayoutManager(this).apply { this.orientation = LinearLayoutManager.VERTICAL }
                ...
                rvRight.addOnItemTouchListener(object : RecyclerView.OnItemTouchListener {
                    override fun onTouchEvent(thisRv: RecyclerView?, e: MotionEvent?) {
                        if (e?.action == MotionEvent.ACTION_MOVE || e?.action == MotionEvent.ACTION_UP || e?.action == MotionEvent.ACTION_DOWN) {
                            ...
                            val holder =
                                thisRv.findContainingViewHolder(view) as CountryRightAdapter.CountryRightHolder?
                                    ?: return
    ...
    private inner class CountryAdapter(val list: MutableList<CountryBean>) :
    ...
    private inner class CountryRightAdapter(private val map: TreeMap<String, Int>) :
        RecyclerView.Adapter<CountryRightAdapter.CountryRightHolder>() {

        override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): CountryRightHolder {
            return CountryRightHolder(
                LayoutInflater.from(parent!!.context).inflate(
                    R.layout.item_lt_country_right,
                    parent,
                    false
                )
            )
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun getCountryBean(data: Intent?): CountryBean? = data?.getSerializableExtra("bean") as CountryBean?
    }
}